...
const MIN_LIQUIDITY = process.env.MIN_LIQUIDITY; //Its value represents a numeric threshold or minimum amount of liquidity that the script or application should wait for before taking specific actions.
const TOKEN_OUT_NAME = process.env.TOKEN_OUT_NAME; //if TOKEN_OUT_NAME is set to 'ETH', the bot would be looking to swap tokens for Ether (ETH) whenever it detects a suitable trading opportunity.
...
const addresses = {
  ...
  tokenIn: process.env.TOKEN_IN_ADDR, // This variable typically holds the Ethereum address of the token that the bot will use as the input token in swaps or liquidity pool operations.
  router: process.env.ROUTER_ADDR, //This variable holds the Ethereum address of the router contract of the DEX (such as Uniswap or PancakeSwap).
  factory: process.env.FACTORY_ADDR, //The factory contract is responsible for creating new pairs of tokens and emits events when new pairs are created.
};

// instantiating our contracts
const factory = new ethers.Contract(addresses.factory, abi["factory"], signer);
const router = new ethers.Contract(addresses.router, abi["router"], signer);
...
factory.on("PairCreated", async (token0, token1, pairAddress) => {
  ...
  if (outTokenSymbol.toLowerCase() === TOKEN_OUT_NAME.toLowerCase()) {
    console.log(
      `Found ${TOKEN_OUT_NAME}'s Contract Addr... Now Waiting For Liquidity To Be Added`
    );
    ...
    var timer = setInterval(async () => {
      ...
      const tokenInReserves = ethers.utils.formatEther(
        ethers.BigNumber.from(rawReserves[tokenInIndex])
      );
      ...
      if (tokenInReserves >= MIN_LIQUIDITY) {
        ...
        const approve_tokenIn_tx = await tokenInContract.approve(
          addresses.router,
        ...
        console.log(
          `received\t: ${ethers.utils.formatEther(
            await tokenOutContract.balanceOf(signer.address)
          )} ${TOKEN_OUT_NAME}`
        );
        console.log("- ".repeat(12));
